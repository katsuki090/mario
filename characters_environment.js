...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Presiona el boton 'jugar' para iniciar el juego", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
{
  GameStatus = "start";
  document.getElementById("carga").innerHTML = "El juego está cargando";
}
...
function changeGameStatud(character){
 if(noseX !="" && gameConfig.status==="start" && GameStatus=="start") { 
   document.getElementById("carga").innerHTML = "El juego se ha cargado";
   world_start.play();
